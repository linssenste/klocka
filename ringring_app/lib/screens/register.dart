        ...
        child: ListView(
          children: [
            const Padding(
              ...
              child: Text(
                'Es werden einige Informationen benötigt, damit wir Ihr Ladenlokal mit dem QR-Code verbinden können. \nKeine Sorge, die Daten werden lediglich für die QR-Cordes verwendet und in keinem Fall an Dritte weitergeleitet!',
                style: TextStyle(
                  fontSize: 16.0,
                  color: Colors.black,
                ),
                                ...
                                ? () async {
                                    ...
                                    if (response.statusCode != 200) {
                                      ...
                                      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                                          content: Text(
                                              "Leider ist ein Fehler aufgetreten! Probieren Sie es doch noch einmal.")));
                                    } else {
                                      print('Response body: ${response.body}');
                                      await FirebaseMessaging.instance.subscribeToTopic(code.toString());
                                      StorageService.companyName = companyName;
                                      StorageService.contactName = contactName;
                                      StorageService.streetName = address;
                                      StorageService.zipCode = zipCode;
                                      StorageService.cityName = cityName;

                                      StorageService.isSubscribed = true;
                                      StorageService.companyLink = code.toString();
                                      StorageService.companyDetails = companyName;
