...
exports.registerCompany = exports.createSticker = exports.ring = exports.login = exports.checkExistence = void 0;
/* eslint-disable */
const axios_1 = require("axios");
const firebase = require("firebase-admin");
const bcrypt = require("bcrypt");
const names_1 = require("../assets/names");
const admin = firebase;
const db = firebase.firestore();
// const { mdToPdf } = require('md-to-pdf');
async function getCodeData(entryId) {
    const localReference = db.collection("codes").doc(entryId);
    return await localReference.get();
}
async function getDatabaseEntry(entryId) {
    const localReference = db.collection("Lokale").doc(entryId);
    return await localReference.get();
}
async function setDatabaseEntry(entryId, payload) {
    try {
...
async function createIdentifier() {
    ...
    const getIdentifier = () => { return names_1.animalNames[Math.floor(Math.random() * names_1.animalNames.length)]; };
...
async function createSticker(req, res) {
    var qrCodeData = await createIdentifier();
    const apiUrl = `http://api.qrserver.com/v1/create-qr-code/?data=${qrCodeData.url}&size=${qrCodeData.size}x${qrCodeData.size}&ecc=H&`;
    try {
        // reformat png response to base64 style
        const reqResult = await axios_1.default.get(apiUrl, { responseType: 'arraybuffer' });
